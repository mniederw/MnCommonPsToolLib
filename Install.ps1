...
function InstallSrcPathToPsModulePathIfNotInstalled ( [String] $srcDir ){ OutProgress "Change environment system variable $envVar by appending '$srcDir'. "; if( (PsModulePathContains $srcDir) ){ OutProgress "Already installed so environment variable not changed."; }else{ ProcessRestartInElevatedAdminMode; PsModulePathAdd $srcDir; } }
...
OutProgress "  required and this tool automatically prompts for it. ";
...
OutInfo    "  U = Uninstall. ";
OutInfo    "  W = When installed in standard mode do update from web otherwise warning. ";
if( DirExists "$srcRootDir\.git" ){ OutInfo    "  C = For developer: actualize sha2 hash file of library. "; }
OutInfo    "  Q = Quit. `n";
if( $sel -ne "" ){ OutProgress "Selection: $sel "; }
while( @("I","A","U","W","Q","C") -notcontains $sel ){
  OutQuestion "Enter selection and press enter (case insensitive: I,A,U,Q): ";
...
if( $sel -eq "W" ){ MnCommonPsToolLibSelfUpdate; }
