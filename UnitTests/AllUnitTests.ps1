...
OutProgress "  Each existing unit test file with the pattern `"$PSScriptRoot/MnCommonPsToolLib*`" ";
OutProgress "  will be called separatedly and on failure it shows the error and continues with the other ones and at the end it throws the summary. ";
...
if( $errorPs1Files.Count -gt 0 ){ throw [ExcMsg] "AllUnitTests failed for the $($errorPs1Files.Count) files:`n  $(StringArrayConcat (StringArrayDblQuoteItems $errorPs1Files) '`n  ') "; }
AssertRcIsOk;
